...
function fullChangelogUrl(v1, v2) {
	return `https://github.com/arpsystems/arpsystems/compare/v${v1}...v${v2}`;
}
...
class RepositoryFetcher {
	...
	// organization https://github.com/arpsystems.
	...
	async fetchTaggedCommit(tag) {
		const tagQuery = `query fetchTaggedCommit($repositoryName: String!, $tag: String!) {
			repository(owner: "arpsystems", name: $repositoryName) {
...
async function generateChangelogEntry(changelog, targetVersion) {
	...
	const codeRepo = new RepositoryFetcher(client, "arpsystems");
	...
	if (isPrerelease(targetVersion)) {
		...
		const websiteRepo = new RepositoryFetcher(client, "arpsystems.github.io");
